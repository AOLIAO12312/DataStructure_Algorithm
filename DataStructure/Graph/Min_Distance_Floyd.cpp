#include<iostream>
#include<string>
#include<assert.h>
#include<queue>
#include<limits>
#define MAX_SIZE 10
using namespace std;
//邻接矩阵法
//带权路径图
class Vertex {
    public:
    string Name;
    bool isOccupied;
    Vertex(string name) {
        Name = name;
        isOccupied = true;
    }
    Vertex() {
        Name = "null";
        isOccupied = false;
    }
};
class AMGraph {//Adjacency Matrix Graph
    private:
    int Matrix[MAX_SIZE][MAX_SIZE];
    Vertex vertices[MAX_SIZE];
    bool visited[MAX_SIZE];
    int size;
    public:
    AMGraph() {
        for (int i = 0;i < MAX_SIZE;++i) {
            for (int j = 0;j < MAX_SIZE;++j) {
                Matrix[i][j] = (i == j) ? 0 : INT_MAX;
            }
        }
        size = 0;
    }
    ~AMGraph() {}
    bool Adjacent(int x, int y) {
        return Matrix[x][y];
    }//判断图G是否存在边（x，y）(x,y)表示节点
    void Neighbors(int x) {
        assert(vertices[x].isOccupied == true);
        for (int i = 0;i < size;++i) {
            if (vertices[i].isOccupied == true) {
                if (Matrix[x][i]) {
                    cout << vertices[i].Name << " ";
                }
            }
        }
        cout << endl;
    }//列出图G与x邻接的顶点
    bool InsertVertex(int x, string Name) {
        assert(x < MAX_SIZE && x >= 0);
        if (vertices[x].isOccupied == false) {
            vertices[x].isOccupied = true;
            vertices[x].Name = Name;
            ++size;
            return true;
        } else {
            cout << x << " position has been occupied" << endl;
            return false;
        }

    }//在图G插入顶点x
    bool DeleteVertex(int x) {
        assert(x < MAX_SIZE && x >= 0);
        if (vertices[x].isOccupied == true) {
            vertices[x].isOccupied == false;
            for (int i = 0;i < MAX_SIZE;++i) {
                Matrix[i][x] = 0;
                Matrix[x][i] = 0;//清空路径数据
            }
            --size;
            return true;
        } else {
            return false;
        }
    }//删除顶点
    bool AddEdge(int x, int y,int weight) {//单向插入由x顶点到y顶点的路径
        Matrix[x][y] = weight;
        return true;
    }//添加链接x，y的边
    int FirstNeighbor(int x) {
        assert(vertices[x].isOccupied == true);
        for (int i = 0;i < size;++i) {
            if (vertices[i].isOccupied == true) {
                if (Matrix[x][i]) {
                    return i;
                }
            }
        }
        return -1;
    }//找到与x相连的第一个节点，没有则返回-1
    int NextNeighbor(int x, int y) {//寻找x顶点y之后的邻接点，没有则返回-1
        assert(vertices[x].isOccupied == true);
        for (int i = y + 1;i < size;++i) {
            if (vertices[i].isOccupied == true) {
                if (Matrix[x][i]) {
                    return i;
                }
            }
        }
        return -1;
    }
    void Show() {
        cout << "\t";
        for (int i = 0;i < size;++i) {
            cout << vertices[i].Name << "\t";
        }
        cout << endl;
        cout << endl;
        for (int i = 0;i < size;++i) {
            cout << vertices[i].Name << "\t";
            for (int j = 0;j < size;++j) {
                if (Matrix[i][j] == INT_MAX) {
                    cout << '*' << "\t";
                } else {
                    cout << Matrix[i][j] << "\t";
                }
            }
            cout << endl;
        }
    }
    void visit(int v) {
        cout << vertices[v].Name << " " << v << endl;
    }
    void BFSTraverse() {//遍历插入的所有节点（包括非连通图），广度优先遍历
        for (int i = 0;i < MAX_SIZE;++i) {
            visited[i] = false;
        }
        for (int i = 0;i < size;++i) {
            if (!visited[i]) {
                BFS(i);
            }
        }
    }
    void BFS(int v) {//从顶点v出发，广度优先遍历图G    
        queue<int> qv;
        visit(v);
        visited[v] = true;
        qv.push(v);
        while (!qv.empty()) {
            v = qv.front();
            qv.pop();
            for (int w = FirstNeighbor(v);w >= 0;w = NextNeighbor(v, w)) {
                //检测v的所有邻接点
                if (!visited[w]) {
                    visit(w);
                    visited[w] = true;
                    qv.push(w);
                }
            }
        }
    }
    void DFSTraverse() {//深度优先遍历
        for (int i = 0;i < MAX_SIZE;++i) {
            visited[i] = false;
        }
        for (int i = 0;i < size;++i) {
            if (!visited[i]) {
                DFS(i);
            }
        }
    }
    void DFS(int v) {
        visit(v);
        visited[v] = true;
        for (int w = FirstNeighbor(v);w >= 0;w = NextNeighbor(v, w)) {
            if (!visited[w]) {
                DFS(w);
            }
        }
    }
    void BFS_MIN_Distance(int u) {//通过BFS计算无权最短路径问题
        assert(u < size);
        int* d = new int[size];//存储距离指定顶点的距离
        int* path = new int[size];//存储该顶点的上一顶点
        queue<int> qv;
        for (int i = 0;i < MAX_SIZE;++i) {//初始化visited数组
            visited[i] = false;
        }
        for (int i = 0;i < size;++i) {
            d[i] = INT_MAX;
            path[i] = -1;
        }
        d[u] = 0;
        visited[u] = true;
        qv.push(u);
        while (!qv.empty()) {
            u = qv.front();
            qv.pop();
            for (int w = FirstNeighbor(u);w >= 0;w = NextNeighbor(u, w)) {
                if (!visited[w]) {
                    d[w] = d[u] + 1;//对上一个节点路径加一
                    path[w] = u;//记录上一节点
                    visited[w] = true;
                    qv.push(w);
                }
            }
        }
        for (int i = 0;i < size;++i) {
            cout << d[i] << " ";
        }
        cout << endl;
        for (int i = 0;i < size;++i) {
            cout << path[i] << " ";
        }
        cout << endl;

        delete[] d;
        delete[] path;
    }

    void Dijkstra(int u) {//从顶点u开始计算
        
        bool* final = new bool[size];
        long long* dist = new long long[size];
        long long* path = new long long[size];
        for (int i = 0;i < size;++i) {
            final[i] = false;
            dist[i] = Matrix[u][i];
            path[i] = (Matrix[u][i] == INT_MAX) ? -1 : 0;
        }
        final[u] = true;
        dist[u] = 0;
        while ([&]() {for (int i = 0;i < size;++i) { if (final[i] == false)return true; }return false;}()) {
            int Min = INT_MAX;
            int Pos = -1;
            for (int i = 0;i < size;++i) {
                if (final[i] == false && Min >= dist[i]) {
                    Pos = i;
                    Min = dist[i];
                }
            }
            final[Pos] = true;
            u = Pos;
            for (int i = 0;i < size;++i) {
                if (final[i] == false && dist[i] > dist[u] + Matrix[u][i]) {
                    dist[i] = dist[u] + Matrix[u][i];
                    path[i] = u;
                }
            }

        }
        for (int i = 0;i < size;++i) {
            cout << dist[i] << " ";
        }
        cout << endl;
        for (int i = 0;i < size;++i) {
            cout << path[i] << " ";
        }
        cout << endl;
        delete[] final;
        final = nullptr;
        delete[] dist;
        dist = nullptr;
        delete[] path;
        path = nullptr;
    }

    void Floyd() {
        long long A[size][size];
        int path[size][size];
        for (int i = 0;i < size;++i) {
            for (int j = 0;j < size;++j) {
                A[i][j] = Matrix[i][j];
                path[i][j] = -1;
            }
        }//初始化path数组和A数组（最短路径长度）
        for (int k = 0;k < size;++k) {
            for (int i = 0;i < size;++i) {
                for (int j = 0;j < size;++j) {
                    if (A[i][j] > A[i][k] + A[k][j]) {
                        A[i][j] = A[i][k] + A[k][j];//最短路径长度更新
                        path[i][j] = k;//中转点更新
                    }
                }
            }
        }

        //打印数组
        for (int i = 0;i < size;++i) {
            for (int j = 0;j < size;++j) {
                if (A[i][j] != INT_MAX) {
                    cout << A[i][j] << " ";
                }else{
                    cout << "* ";
                }
            }
            cout << endl;
        }
        cout << endl;
        for (int i = 0;i < size;++i) {
            for (int j = 0;j < size;++j) {
                cout << path[i][j] << " ";
            }
            cout << endl;
        }
    }
};
int main() {
    AMGraph g;
    for (int i = 0;i < 5;++i) {
        g.InsertVertex(i, string(1, 'A' + i));
    }
    g.AddEdge(0, 2, 1);
    g.AddEdge(2, 1, 1);
    g.AddEdge(0, 4, 10);
    g.AddEdge(1, 4, 5);
    g.AddEdge(1, 3, 1);
    g.AddEdge(3, 4, 1);
    g.AddEdge(2, 4, 7);
    g.Floyd();
    return 0;
}